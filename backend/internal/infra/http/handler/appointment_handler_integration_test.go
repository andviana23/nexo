...
func setupApptHandler(t *testing.T) (*handler.AppointmentHandler, *echo.Echo, func()) {
	...
	// Use cases
	createUC := appointment.NewCreateAppointmentUseCase(appointmentRepo, serviceReader, professionalReader, customerReader, logger)
	...
	updateStatusUC := appointment.NewUpdateAppointmentStatusUseCase(appointmentRepo, logger)
	rescheduleUC := appointment.NewRescheduleAppointmentUseCase(appointmentRepo, logger)

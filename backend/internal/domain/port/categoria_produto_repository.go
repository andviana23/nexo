...
type CategoriaProdutoRepository interface {
	...
	FindByID(ctx context.Context, tenantID, id uuid.UUID) (*entity.CategoriaProdutoEntity, error)

	// FindByNome busca uma categoria por nome (único por tenant)
	FindByNome(ctx context.Context, tenantID uuid.UUID, nome string) (*entity.CategoriaProdutoEntity, error)

	// ListAll lista todas as categorias do tenant
	ListAll(ctx context.Context, tenantID uuid.UUID) ([]*entity.CategoriaProdutoEntity, error)

	// ListAtivas lista apenas categorias ativas do tenant
	ListAtivas(ctx context.Context, tenantID uuid.UUID) ([]*entity.CategoriaProdutoEntity, error)
	...
	ExistsWithNome(ctx context.Context, tenantID uuid.UUID, nome string, excludeID *uuid.UUID) (bool, error)

	// CountProdutosVinculados conta quantos produtos estão vinculados à categoria
	CountProdutosVinculados(ctx context.Context, tenantID, categoriaID uuid.UUID) (int64, error)
}
